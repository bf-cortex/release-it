...
const commit = ({ path = '.', message, version, changelog, args = '' }) =>
  run(`git commit --message="${format(message, version, changelog)}" ${args}`, path).catch(err => {
    debugGit(err);
...
const tag = (version, tag, annotation, changelog) => {
  const force = config.isForce ? '--force ' : '';
  const formattedVersion = format(tag, version);
  const message = format(annotation, version, changelog);
